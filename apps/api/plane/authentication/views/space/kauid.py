# Kaupeh Pte Ltd — KauID OAuth views (space)
...
from plane.authentication.provider.oauth.kauid import KauIDOAuthProvider
...
class KauIDOauthInitiateSpaceEndpoint(View):
...
class KauIDCallbackSpaceEndpoint(View):
    def get(self, request):
        ...
        if state != request.session.get("state", ""):
            exc = AuthenticationException(
                ...
                error_message="KAUID_OAUTH_PROVIDER_ERROR",
            )
            params = exc.get_error_dict()
            if next_path:
                params["next_path"] = str(validate_next_path(next_path))
